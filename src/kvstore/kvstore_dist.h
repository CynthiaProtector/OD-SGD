...
//====================== begin xym edit this 4.23 ======================//
  ...
  void PullBroadImpl(const std::vector<int>& keys,
      ...
      for (size_t i = 0; i < uniq_keys.size(); ++i) {
        ...
        CopyFromTo(comm_buf_[key], &comm_buf_copy[key]); // xym edit 4.23;
      }
  }
...
//====================== end   xym edit this 4.1 ======================//
  ...
  void PullImpl(const std::vector<int>& keys,
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      auto &recv_buf_copy= comm_buf_copy[key]; // xym add this
      ...
      if(pull_first_time/parameter_count <= 200 ) {
        ...
        if(pull_first_time/parameter_count == 200)
          CopyFromTo(recv_buf, &recv_buf_copy); // xym edit 4-15;
      } else {
        CHECK_NOTNULL(Engine::Get())->PushAsync(
                pull_from_servers,
                pinned_ctx_,
  ...
  void Push_(const std::vector<int>& keys,
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      auto &local_comm = comm_local_copy[key];// xym add this 5.20 to store
      auto &update_local = comm_buf_copy[key]; // xym add this 5.23  to store the comm_buf_copy

      // xym add this 5.21 malloc the memory for local_comm
...
// 之前是放在push操作前面的，现在放到后面；看看实验效果
//*************** xym add this to update the local weights 5.20 begin ******************
      if (updater_ != nullptr && pull_first_time/parameter_count > 200) {
// This part update the weights locally
        if (key_type_ == kStringKey && str_updater_ != nullptr) {
          ...
          str_updater_(str_key, send_buf,  &update_local);
        } else {
          updater_(key, send_buf,  &update_local);
        }
      }
//*************** xym add this to update the local weights 5.20 end *******************
  ...
  /** \brief xym add this 5.06
  */
  std::unordered_map<int, NDArray> comm_buf_copy;
  std::unordered_map<int, NDArray> comm_buf_bk;
...
//  int parameter_count = 156; //Resnet-50
...
//  int parameter_count = 64;  //ResNet20
//  int parameter_count = 108;  //ResNet34_v2
//  int parameter_count = 157;  //ResNet50_v2
